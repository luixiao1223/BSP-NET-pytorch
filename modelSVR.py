...
class resnet_block(nn.Module):
    def __init__(self, dim_in, dim_out):
        ...
        if self.dim_in == self.dim_out:
            self.conv_1 = nn.Conv2d(self.dim_in,
                                    self.dim_out,
                                    3,
                                    stride=1,
                                    padding=1,
                                    bias=False)
            self.conv_2 = nn.Conv2d(self.dim_out,
                                    self.dim_out,
                                    3,
                                    stride=1,
                                    padding=1,
            ...
            self.conv_2 = nn.Conv2d(self.dim_out,
                                    ...
                                    padding=1,
                                    bias=False)
            self.conv_s = nn.Conv2d(self.dim_in,
                                    self.dim_out,
                                    1,
                                    stride=2,
                                    padding=0,
...
class img_encoder(nn.Module):
    def __init__(self, img_ef_dim, z_dim):
        ...
        self.conv_9 = nn.Conv2d(self.img_ef_dim * 8,
                                self.img_ef_dim * 16,
                                4,
                                stride=2,
                                padding=1,
                                bias=True)
        self.conv_10 = nn.Conv2d(self.img_ef_dim * 16,
                                 self.img_ef_dim * 16,
                                 4,
                                 stride=1,
                                 padding=0,
                                 bias=True)
        self.linear_1 = nn.Linear(self.img_ef_dim * 16,
                                  self.img_ef_dim * 16,
                                  bias=True)
        self.linear_2 = nn.Linear(self.img_ef_dim * 16,
                                  self.img_ef_dim * 16,
                                  bias=True)
        self.linear_3 = nn.Linear(self.img_ef_dim * 16,
                                  self.img_ef_dim * 16,
                                  bias=True)
...
class decoder(nn.Module):
    def __init__(self, ef_dim, p_dim):
        ...
        self.linear_2 = nn.Linear(self.ef_dim * 16,
                                  self.ef_dim * 32,
                                  bias=True)
        self.linear_3 = nn.Linear(self.ef_dim * 32,
                                  self.ef_dim * 64,
                                  bias=True)
    ...
    def forward(self,
        ...
        if is_training:
            ...
            if point_coord is not None:
                net_out_convexes, net_out = self.generator(
                    point_coord, plane_m, is_training=is_training)
            else:
                net_out_convexes = None
                net_out = None
...
class BSP_SVR(object):
    def __init__(self, config):
        ...
        if os.path.exists(data_hdf5_name):
            ...
            self.data_pixels = np.reshape(
                data_dict['pixels'][:, :, offset_y:offset_y +
                                    self.crop_size, offset_x:offset_x +
                                    self.crop_size],
        ...
        self.coords = np.reshape(self.coords,
                                 [multiplier3, test_point_batch_size, 3])
        ...
        #build model
        self.bsp_network = bsp_network(self.ef_dim, self.p_dim, self.c_dim,
                                       self.img_ef_dim, self.z_dim)
        ...
        self.optimizer = torch.optim.Adam(
            self.bsp_network.img_encoder.parameters(),
        ...
        self.checkpoint_AE_path = os.path.join(self.checkpoint_dir,
                                               self.modelAE_dir)
    ...
    @property
    def model_dir(self):
        return "{}_svr_{}".format(self.dataset_name, self.crop_size)
    ...
    def train(self, config):
        ...
        for epoch in range(0, training_epoch):
            ...
            for idx in range(batch_num):
                dxb = batch_index_list[idx * self.shape_batch_size:(idx + 1) *
                ...
                z_vector, _, _, _ = self.bsp_network(batch_view,
                                                     None,
                                                     None,
                                                     None,
                                                     is_training=True)
            ...
            if epoch % 100 == 99:
                ...
                save_dir = os.path.join(
                    ...
                    self.checkpoint_name + "-" + str(epoch) + ".pth")
                self.checkpoint_manager_pointer = (
                    self.checkpoint_manager_pointer + 1) % self.max_to_keep
                #delete checkpoint
                if self.checkpoint_manager_list[
                        self.checkpoint_manager_pointer] is not None:
                    if os.path.exists(self.checkpoint_manager_list[
                            self.checkpoint_manager_pointer]):
                        os.remove(self.checkpoint_manager_list[
                            self.checkpoint_manager_pointer])
                #save checkpoint
                torch.save(self.bsp_network.state_dict(), save_dir)
                #update checkpoint manager
                self.checkpoint_manager_list[
                    self.checkpoint_manager_pointer] = save_dir
                #write file
                checkpoint_txt = os.path.join(self.checkpoint_path,
                                              "checkpoint")
        ...
        save_dir = os.path.join(
            self.checkpoint_path,
            self.checkpoint_name + "-" + str(training_epoch) + ".pth")
        self.checkpoint_manager_pointer = (self.checkpoint_manager_pointer +
                                           1) % self.max_to_keep
        #delete checkpoint
        if self.checkpoint_manager_list[
                self.checkpoint_manager_pointer] is not None:
        ...
        fout = open(checkpoint_txt, 'w')
        for i in range(self.max_to_keep):
            pointer = (self.checkpoint_manager_pointer + self.max_to_keep -
                       i) % self.max_to_keep
            if self.checkpoint_manager_list[pointer] is not None:
                fout.write(self.checkpoint_manager_list[pointer] + "\n")
        fout.close()
        ...
        for i in range(multiplier):
            for j in range(multiplier):
                for k in range(multiplier):
                    ...
                    model_float[self.aux_x + i + 1, self.aux_y + j +
                                1, self.aux_z + k + 1] = np.reshape(
            ...
            for i in range(multiplier):
                for j in range(multiplier):
                    for k in range(multiplier):
                        ...
                        model_float[self.aux_x + i, self.aux_y +
